// backend/scripts/initialize-bandejas.js
...
async function initializeBandejas() {
  try {
    ...
    // Create 10 bandejas
    for (let i = 1; i <= 10; i++) {
      // Using the bandeja number as the document ID
      const bandejaRef = doc(db, "bandejas", i.toString());

      // Default values for a new bandeja
      const bandejaData = {
        id_bandeja: i.toString(),
      ...
      console.log(
        `✅ Created bandeja ${i} with document ID: ${i}, id_bandeja: "${i}"`
      );
    }

    // Create the almacen document
    const almacenRef = doc(db, "almacen", "almacen");
    ...
    console.log(`✅ Created almacen with ID: almacen`);
